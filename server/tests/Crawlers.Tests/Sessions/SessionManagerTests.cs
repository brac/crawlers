...
public class SessionManagerTests
{
    ...
    {
        var mgr = new SessionManager();
        var state = mgr.CreateSoloSession(seed: 7);
    ...
    {
        var mgr = new SessionManager();
        var a = mgr.CreateSoloSession(seed: 1);
        var b = mgr.CreateSoloSession(seed: 2);

        Assert.NotEqual(a.Session.Id, b.Session.Id);
        Assert.NotEqual(a.PrimaryPlayer.Id, b.PrimaryPlayer.Id);
        Assert.NotEqual(a.GetFloorFor(a.PrimaryPlayer).Id, b.GetFloorFor(b.PrimaryPlayer).Id);
    ...
    {
        var mgr = new SessionManager();
        var created = mgr.CreateSoloSession(seed: 1);
    ...
    public void Multi_player_session_seats_each_player_on_a_distinct_walkable_tile()
    {
        ...
        var state = mgr.CreateSession(starts, seed: 11);
    ...
    {
        var mgr = new SessionManager();
        var state = mgr.CreateSoloSession(seed: 5);
    ...
    public void Shared_fog_is_per_floor_and_unions_each_players_LOS()
    {
        ...
        var starts = Enumerable.Range(0, 2)
            ...
            .ToList();
        var state = mgr.CreateSession(starts, seed: 9);
    ...
    {
        var mgr = new SessionManager();
        var state = mgr.CreateSoloSession(seed: 11);
        var fog = state.GetFog(1)!;
    ...
    public void Multi_floor_session_creation_seats_each_cohort_on_their_starting_floor()
    {
        ...
        var state = mgr.CreateSession(new[] { floor1Start, floor3Start }, seed: 17);
